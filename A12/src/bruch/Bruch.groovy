...
class Bruch implements Comparable{
	...
	/**
	 * Divides 2 fractions by multiplying each counter and denominator
	 * 
	 * @param other the fraction or number that this fraction is getting divided by
	 * 
	...
	Bruch div(other, optional=null){
		...
		if(other instanceof Bruch){
			this.z *= other.n
			this.n *= other.z
		// calculate denominator if argument is single number, counter stays the same
	 ...
	 * 
	 * @param other the fraction or number that this fraction is getting multiplied by
	 * 
	 * @param optional this parameter exists, because it has to be possible to call
	 * this method with 2 numbers instead of a fraction or a single number,
	 * in this case, other becomes the counter and optional the denominator
	...
	Bruch multiply(other, optional=null){
		...
		if(other instanceof Bruch){
			this.z *= other.z
			this.n *= other.n
		// calculate counter if argument is single number, denominator stays the same
		} else if (optional==null){
			this.z *= other
		// calculate counter and denominator if argument is 2 numbers
		} else {
	...
	/**
	 * Adds 2 fractions by finding the least common multiple and then adding
	 * counters accordingly
	 * 
	 * @param other the fraction or number that this fraction is getting added to
	 * 
	 * @param optional this parameter exists, because it has to be possible to call
	 * this method with 2 numbers instead of a fraction or a single number,
	 * in this case, other becomes the counter and optional the denominator
	 * 
	 * @return this fraction added to the other fraction
	 */
	Bruch plus(other, optional=null){
		// calculate counter and denominator if argument is a fraction
		if(other instanceof Bruch){
			def lcm = lcm(this.n,other.n)
			
			this.z = (this.z * lcm / this.n) + (other.z * lcm / other.n as BigInteger)
			this.n = lcm
		// calculate counter if argument is single number, denominator stays the same
		} else if(optional==null){
			this.z = this.z + (this.n*other)
		// calculate counter and denominator if argument is 2 numbers
		} else {
			def lcm = lcm(this.n, optional)
			
			this.z = (this.z * lcm / this.n) + (other * lcm / optional) as BigInteger
			this.n = lcm
		}
		new Bruch(this.z,this.n)
	}
	...
	/**
	 * Subtracts 2 fractions by finding the least common multiple and then subtracting
	 * counters accordingly
	 *
	 * @param other the fraction or number that this fraction is getting subtracted by
	 *
	 * @param optional this parameter exists, because it has to be possible to call
	 * this method with 2 numbers instead of a fraction or a single number,
	 * in this case, other becomes the counter and optional the denominator
	 *
	 * @return this fraction subtracted by the other fraction
	 */
	...
	/**
	 * Correct the denominator and counter if the denominator is negative instead
	 * of the counter
	 */
	void correct(){
		if(this.n < 0){
	...
	/**
	 * 
	 * @return a Bruch where the counter is negative
	 */
	Bruch negative(){
		new Bruch(-this.z, this.n)
	}
	...
	/**
	 * shorten the fraction using the greatest common divisor
	 */
	void shorten(){
	 ...
	 * 
	 * @return the least common multiple using the gcd
	 */
	...
	/**
	 * Override the compareTo() function for the spaceship operator (<=>)
	 */
	@Override
	...
	/**
	 * Override the equals() function for the JUnit comparions
	 */
	@Override
